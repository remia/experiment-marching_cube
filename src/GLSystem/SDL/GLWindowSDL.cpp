...
GLWindowSDL::GLWindowSDL(int argc, char** argv)
...
void GLWindowSDL::SDLInitialize()
{
	_init();
}

void GLWindowSDL::SDLResize(int width, int height)
{
...
void GLWindowSDL::SDLPaint()
{
	_paint();
	SDL_GL_SwapBuffers();
}

void GLWindowSDL::ProcessEvents()
{
	SDL_Event event;
...
void GLWindowSDL::SDLMouseEvent(int buttons, int state, int x, int y)
{
	_mouseEvent(buttons, state, x, y);
}

void GLWindowSDL::SDLMouseMove(int x, int y)
{
	_mouseMove(x, y);
}
